    ...
    public static class SnoSearchProxy
    {
        ...
        public static T GetRecord<T>(SnoType type, int snoId) where T : MemoryWrapper, new()
        {
            ...
            switch (type)
            {
                case SnoType.Actor:
                    if (_actorTable == null)
                        _actorTable = Pointers.Addresses.ActorSnoTable.UnsafeCreate<SNOTable>(ClientSNOTable.Actor, LeastOverheadRecordFactory);
                    table = _actorTable;
                    break;

                case SnoType.Power:
                    if (_powerTable == null)
                        _powerTable = Pointers.Addresses.PowerSnoTable.UnsafeCreate<SNOTable>(ClientSNOTable.Power, LeastOverheadRecordFactory);
                    table = _powerTable;
                    break;
