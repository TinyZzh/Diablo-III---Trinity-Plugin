...
namespace QuestTools.Helpers
{
    ...
    public static class BotBehaviorQueue
    {
        ...
        public static class BehaviorTreeExecutor
        {
            ...
            {
                Logger.Warn("Spooling up new Bot thread");

                var isStarted = false;
                var tag = new RandomMoveTag();
                ...
                (new Worker()).Start(() =>
                {
                    ...
                    {
                        Logger.Warn("Work is finished, Shutting down Bot thread");
                        return true;
                    }
            ...
            private static void Tick()
            {
                ...
                {
                    Logger.Log("Exception in BehaviorTreeExecutor.Logic.Tick() - {0}", ex);
                    _logic.Stop(null);
                    _logic.Start(null);
                    throw;
                }
