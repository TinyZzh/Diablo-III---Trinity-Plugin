    ...
    public class LoadOnceTag : ProfileBehavior
    {
        ...
        protected override Composite CreateBehavior()
        {
            return new Sequence(
                ...
                new Action(ret => Logger.Log("LoadOnce: Found {0} Total Profiles, {1} Used Profiles, {2} Unused Profiles",
                    Profiles.Count(), UsedProfileCount, UnusedProfileCount)),
                new PrioritySelector(
                    new Decorator(ret => AvailableProfiles.Length == 0,
                        new Sequence(
                            new Action(ret => Logger.Log("LoadOnce: All available profiles have been used!", true)),
                        ...
                        new Sequence(
                            ...
                            new PrioritySelector(
                                new Decorator(ret => File.Exists(_nextProfilePath),
                                    new Sequence(
                                        new Action(ret => Logger.Log("LoadOnce: Loading next profile: {0}", _nextProfileName)),
                                        new Action(ret => UsedProfiles.Add(_nextProfileName)),
                                        new Action(ret => ProfileManager.Load(_nextProfilePath))
                                    )
                                ),
                                new Action(ret => Logger.Log("LoadOnce: ERROR: Profile {0} does not exist!", _nextProfilePath))
                            )
                        )
                    ),
