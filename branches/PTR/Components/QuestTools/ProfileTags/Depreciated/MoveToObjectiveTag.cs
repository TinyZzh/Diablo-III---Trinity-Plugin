...
namespace QuestTools.ProfileTags.Movement
{
    ...
    public class MoveToObjectiveTag : ProfileBehavior, IEnhancedProfileBehavior
    {
        ...
        protected async Task<bool> MoveToObjectiveRoutine()
        {
            ...
            {
                Logger.Log("IsDead={0} IsLoadingWorld={1}", ZetaDia.Me.IsDead, ZetaDia.Globals.IsLoadingWorld);
                return false;
            }
            ...
            {
                Logger.Log("World changed from {0} to {1}, finished {2}", _startWorldId, ZetaDia.Globals.WorldSnoId, Status());
                _isDone = true;
                return true;
            }
            ...
            if (_objectiveObject != null && _objectiveObject.IsFullyValid())
            {
                ...
                {
                    Logger.Debug("Moving to actor {0} {1}", _objectiveObject.ActorSnoId, Status());
        ...
        private bool MoveAlongRoute(List<Vector3> route)
        {
            ...
            if (removed > 0)
                Logger.Log("Cleared {0} Nodes within {1} yards", removed, range);
            ...
            {
                Logger.Log("Scene data is now revealed, clearing route");
            ...
            if ((_currentRouteDestination == Vector3.Zero || _currentRouteDestination.Distance(ZetaDia.Me.Position) < range || _lastMoveResult == MoveResult.ReachedDestination || _lastMoveResult == MoveResult.Failed))
            {
                if (route.Any())
                {
                    Logger.Log("Starting New Node", _lastMoveResult);
