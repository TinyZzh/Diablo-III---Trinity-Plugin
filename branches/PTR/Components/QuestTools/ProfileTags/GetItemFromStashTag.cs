...
namespace QuestTools.ProfileTags
{
    ...
    public class GetItemFromStashTag : ProfileBehavior, IEnhancedProfileBehavior
    {
        ...
        private async Task<bool> GetItemFromStashRoutine()
        {
            ...
            {
                Logger.Log("Already have {0} items in our backpack (GameBalanceId={1} ActorSnoId={2} GreaterRiftKey={3})", backPackCount, GameBalanceId, ActorId, GreaterRiftKey);
                _isDone = true;
                return true;
            }
            ...
            if (UIElements.StashWindow.IsVisible)
            {
                ...
                {
                    Logger.Error("Unable to find item in stash with ActorSnoId {0}", ActorId);
                    invalidActorId = true;
                }
